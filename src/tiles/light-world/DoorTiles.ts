...
class ForegroundTile extends Tile {
  ...
  constructor(texture: Texture, adjustment: {x: number; y: number} = {x: 0, y: 0}, opts: TileOpts = {}) {
    super(texture, opts);
    this.adjustment = adjustment;
  }

  setTransform(x: number, y: number): DisplayObject {
...
export function crossTileDoorTiles(
  ...
  const secondBackOfDoorTile = new Tile(secondBackOfDoorTexture, { collisionShape: SECOND_TILE_COLLISION });
  ...
  if (downFacing) {
    [firstBackOfDoorForegroundTexture, secondBackOfDoorForegroundTexture].forEach(texture => {
      texture.frame.height = 8;
      texture.frame.y += 8;
      texture.updateUvs();
    });
    firstBackOfDoorTile.foregroundTile = new ForegroundTile(firstBackOfDoorForegroundTexture, {x: 0, y: 8});
    secondBackOfDoorTile.foregroundTile = new ForegroundTile(secondBackOfDoorForegroundTexture, {x: 0, y: 8});
  } else {
    firstBackOfDoorTile.foregroundTile = new ForegroundTile(firstBackOfDoorForegroundTexture);
    secondBackOfDoorTile.foregroundTile = new ForegroundTile(secondBackOfDoorForegroundTexture);
  }

  return [firstEnterDoorTile, secondEnterDoorTile, firstBackOfDoorTile, secondBackOfDoorTile];
}
