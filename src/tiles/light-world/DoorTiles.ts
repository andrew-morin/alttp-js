...
export function crossTileDoorTiles(loadNextRoom: RoomLoader, doorTextures: Texture[]): [Tile, Tile, Tile, Tile] {
  const [
...
export function singleTileDoorTiles(loadNextRoom: RoomLoader, doorTextures: Texture[]): [Tile, Tile] {
  ...
  const updateOnCollision = function (startDoorTransition: StartDoorTransitionFn): void {
    startDoorTransition(loadNextRoom);
  };
